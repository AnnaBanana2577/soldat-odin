...
team_menu_input :: proc(h: ^Hud, g: ^Game, cursor: sim.Vec2) -> (mouse_taken: bool) {
	t := &h.team
...
@(private = "file")
team_pick :: proc(h: ^Hud, g: ^Game, team: sim.Team) {
	h.team.open = false
	if team != g.world.soldiers[g.me].team do choose_team(g, team)
}

team_menu_draw :: proc(h: ^Hud, g: ^Game, sc: Screen) {
	t := &h.team
	p := TEAM_PANEL
