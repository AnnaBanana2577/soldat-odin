...
connection_open :: proc(c: ^Connection, address: string, port: u16, name: string) -> bool {
	...
	for time.tick_since(deadline) < CONNECT_TIMEOUT {
		...
		case .CONNECT:
			...
			connection_send_message(c, net.Hello{version = net.VERSION, name = name})
		case .RECEIVE:
			reply: net.Message
...
connection_simulate_line :: proc(c: ^Connection, ping, jitter, loss: f64) {
...
connection_receive :: proc(c: ^Connection) -> [][]u8 {
	for p in c.inbox do delete(p)
	clear(&c.inbox)
	event: enet.Event
	...
	if c.fake.on {
		...
		for h in released {
			connection_send_now(c, h.data, h.reliable)
			delete(h.data)
		}
		connection_flush(c)
	}
...
connection_flush :: proc(c: ^Connection) {
	enet.host_flush(c.host)
}

// A message, on the delivery its kind has.
connection_send_message :: proc(c: ^Connection, msg: net.Message) {
	m := msg
	buf: [net.MAX_PACKET]u8
	if size, ok := net.encode(buf[:], &m); ok do connection_send(c, buf[:size], net.RELIABLE[net.message_kind(&m)])
}

connection_send :: proc(c: ^Connection, data: []u8, reliable: bool) {
	if c.fake.on {
	...
	connection_send_now(c, data, reliable)
}

@(private = "file")
connection_send_now :: proc(c: ^Connection, data: []u8, reliable: bool) {
