...
hud_minimap_draw :: proc(h: ^Hud, g: ^Game, sc: Screen, r: ^Render, camera: ^Camera) {
	if !h.minimap_shown do return
	m := minimap(r)
	if m == nil do return
	x, y := spread(sc, MINIMAP_X), MINIMAP_Y
	minimap_draw(m, x, y, sc.scale, MINIMAP_ALPHA)
	...
	for &t in g.world.things {
		...
		dot(h, sc, x, y, minimap_place(m, t.pos[0]), 1, color)
	}
	...
	half := view_half(camera)
	min_x, min_y := minimap_place(m, camera.pos - half).x, minimap_place(m, camera.pos - half).y
	max_x, max_y := minimap_place(m, camera.pos + half).x, minimap_place(m, camera.pos + half).y
