...
debug_frame :: proc(d: ^Debug, dt: f64) {
	...
	if d.menu != "" && !d.menu_done && d.elapsed > 0.5 {
		d.menu_done = true
		hud_open_menu(&client.game.hud, &client.game, d.menu)
	}
	...
	if d.seconds > 0 && d.elapsed > d.seconds && !d.done {
		d.done = true
		things := 0
		for t in client.game.world.things do if t.style != .None do things += 1
		me := &client.game.world.soldiers[client.game.me]
		fmt.printfln("frame time over the second before: %.1f ms; tick %d, %d shots fired, %d hits given and %d taken as seen here, %d things, %d kills, %d deaths, health %.0f, at %.0f,%.0f", d.frame_time / f64(max(d.frames, 1)) * 1000, client.game.world.tick, client.game.shots_fired, client.game.hits_given, client.game.hits_taken, things, me.kills, me.deaths, me.health, me.pos.x, me.pos.y)
		fmt.printfln("the others shown %d ticks behind the server, by its measure", client.game.my_lag)
		g := &client.game
		fmt.printfln("the others shown %d ticks behind the newest word of them", client.game.view.behind)
...
debug_vote :: proc(line: string) {
	...
	case "kick":
		...
		call_kick(&client.game, u8(at), strings.trim_space(rest))
	case "map":
		call_map(&client.game, strings.trim_space(rest))
	case:
