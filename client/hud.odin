...
//
// It reads the game and tells it what the player chose: weapons (choose_weapons),
// a team (choose_team), a line to say (say).
package client
...
hud_init :: proc(h: ^Hud, base: string) {
...
hud_input :: proc(h: ^Hud, g: ^Game, cursor: sim.Vec2) -> (mouse_taken, keys_taken: bool) {
...
hud_draw :: proc(h: ^Hud, g: ^Game, r: ^Render, camera: ^Camera, cursor: sim.Vec2, alpha: f32) {
	sc := screen()
	mine := &g.world.soldiers[g.me]
...
@(private)
draw_status :: proc(h: ^Hud, g: ^Game, sc: Screen, mine: ^sim.Soldier) {
	w := &g.world
	x := spread(sc, 575)
	has_flags := false
...
@(private)
big_kill :: proc(b: ^Big_Message, g: ^Game, kill: sim.Kill) {
	line: string
	switch {
	case kill.killer == g.me && kill.target == g.me: line, b.color = "You killed yourself", {197, 48, 37, 255}
	case kill.killer == g.me: line, b.color = fmt.tprintf("You killed %s", name_of(g, kill.target)), {234, 53, 48, 255}
	case kill.target == g.me: line, b.color = fmt.tprintf("Killed by %s", name_of(g, kill.killer)), {197, 48, 37, 255}
	case: return
	}
...
hud_open_menu :: proc(h: ^Hud, g: ^Game, which: string) {
	switch which {
