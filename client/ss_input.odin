...
input_sample :: proc(in_: ^Input, aim: sim.Vec2, scripted: sim.Buttons, mouse_free, keys_free: bool) {
	held := scripted
	if keys_free {
...
input_command :: proc(in_: ^Input, seq: u32) -> sim.Command {
	return {seq = seq, buttons = in_.held + in_.pressed, aim = in_.aim}
}

input_clear :: proc(in_: ^Input) {
	in_.pressed = {}
}
