...
menus_input :: proc(h: ^Hud, g: ^Game, cursor: sim.Vec2) -> (mouse_taken: bool) {
	m := &h.menus
...
@(private = "file")
menus_press :: proc(h: ^Hud, g: ^Game, menu: Menu, button: int) {
	m := &h.menus
	switch menu {
	case .Esc:
		switch button {
	...
	case .Maps:
		...
		case 2:
			...
			call_map(g, net.text_string(&g.map_list.name))
		}
	case .None:
	}
}
...
@(private = "file")
kick_next :: proc(g: ^Game, from: u8, step: int) -> u8 {
	at := int(from)
...
menus_draw :: proc(h: ^Hud, g: ^Game, sc: Screen) {
	m := &h.menus
	if m.esc {
		left, top := (sc.width - ESC_W) / 2, (480 - ESC_H) / 2
	...
	if m.kick {
		...
		if m.slot < sim.MAX_PLAYERS && g.world.soldiers[m.slot].active {
			text(h, sc, .Menu, name_of(g, m.slot), bx, by - 15, shirt_color(g.world.soldiers[m.slot].team))
		}
