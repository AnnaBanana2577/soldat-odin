...
View :: struct {
	samples: [sim.MAX_PLAYERS][SAMPLES]View_Sample, // each soldier as it was told of, by tick
...
OTHERS_EVERY :: 2 // ticks between the server's words of the others (server/odin)
...
@(private = "file")
view_around :: proc(v: ^View, slot: int, tick: u32) -> (before, after: ^View_Sample) {
...
@(private = "file")
view_show :: proc(ctx: ^sim.Context, s: ^sim.Soldier, before, after: ^View_Sample, part: f32) {
	s^ = before.soldier
