...
@(private = "file")
menu_pick :: proc(m: ^Weapons_Menu, g: ^game.Game, row: int) {
	weapon := menu_weapon(row)
	if row < len(MENU_PRIMARIES) {
		game.choose_weapons(g, weapon, g.secondary)
		m.open = false
	} else {
		game.choose_weapons(g, g.primary, weapon)
	}
}

menu_draw :: proc(h: ^Hud, g: ^game.Game, sc: Screen) {
	m := &h.menu
