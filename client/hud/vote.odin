...
vote_draw :: proc(h: ^Hud, g: ^game.Game, sc: Screen) {
	v := &g.vote
	if !v.active do return
	x, y := VOTE_X, VOTE_Y
	about := v.kind == .Kick ? "Kick" : "Map"
	target := net.text_string(&v.name)
	if v.kind == .Kick && v.target < sim.MAX_PLAYERS do target = game.name_of(g, v.target)

	text(h, sc, .Small, about, x + 30, y, {254, 104, 104, 225})
	text(h, sc, .Small, target, x + 65, y, {244, 244, 244, 225})
	starter := v.starter < sim.MAX_PLAYERS ? game.name_of(g, v.starter) : "Server"
