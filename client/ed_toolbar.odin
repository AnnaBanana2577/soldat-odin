...
layer_parts :: proc(l: Layers) -> Map_Parts {
	parts: Map_Parts
...
draw_toolbar :: proc(e: ^Editor, panes: Panes) {
	...
	x := bar.x + GAP
	y := bar.y + (bar.height - TOOLBAR_BUTTON_H) / 2

	// Opening the list is a toggle, so the button shows whether it is open.
	open := e.layout.list_open
	if rl.GuiToggle({x, y, BUTTON_W, TOOLBAR_BUTTON_H}, "Maps", &open) != 0 {
		e.layout.list_open = open
	}
	x += BUTTON_W + GAP

	if rl.GuiButton({x, y, BUTTON_W, TOOLBAR_BUTTON_H}, "New") do dialog_open_new(e)
	x += BUTTON_W + GAP

	// Nothing to save until a map is open.
	rl.GuiSetState(c.int(e.loaded ? rl.GuiState.STATE_NORMAL : rl.GuiState.STATE_DISABLED))
	if rl.GuiButton({x, y, BUTTON_W, TOOLBAR_BUTTON_H}, "Save") do save_map(e)
	x += BUTTON_W + GAP
	if rl.GuiButton({x, y, BUTTON_W, TOOLBAR_BUTTON_H}, "Fit") {
