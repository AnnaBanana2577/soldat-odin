...
// the way the server's does (server/odin):
//
...
Game :: struct {
	...
	out:       Outbox,    // what I have to tell the server that is not my keys
...
game_receive :: proc(g: ^Game, conn: ^Connection) {
	for data in connection_receive(conn) {
...
vote_yes :: proc(g: ^Game) {
	...
	outbox_vote(&g.out, net.Vote{kind = g.vote.kind, target = g.vote.target, name = g.vote.name})
...
step_mine :: proc(g: ^Game, in_: ^Input) {
	mine := &g.world.soldiers[g.me]
	g.my_prev = mine.pos
	g.seq += 1
	cmd := input_command(in_, g.seq)
...
game_send :: proc(g: ^Game, conn: ^Connection) {
	...
	connection_send_message(conn, m)

	outbox_send(&g.out, conn)
