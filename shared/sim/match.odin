...
// ended counts down to the next (match_over); starting it, on the same map or another,
...
match_tick :: proc(ctx: ^Context, w: ^World, events: ^Events) {
	r := w.match
...
match_end :: proc(w: ^World, events: ^Events) {
	r := w.match
	r.state = .Ended
