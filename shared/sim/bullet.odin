...
Bullet :: struct {
	...
	shot_id:        u32, // its owner's count of its bullets: the same number on every machine
	pos, old_pos:   Vec2,
	vel, forces:    Vec2,
...
bullet_spawn :: proc(ctx: ^Context, w: ^World, pos, vel: Vec2, weapon: Weapon_Id, owner: u8, damage: f32, events: ^Events) -> (index: int, ok: bool) {
	for &b, i in w.bullets {
		...
		w.soldiers[owner].shot_count += 1
		b = {
			active = true, style = info.style, weapon = weapon, owner = owner, lag = w.soldiers[owner].view_lag,
			spawn_cmd = w.soldiers[owner].cmd_seq, shot_id = w.soldiers[owner].shot_count,
...
bullet_end :: proc(w: ^World, b: ^Bullet, index: u16, events: ^Events, impact: Maybe(Vec2) = nil) {
	...
	b.active = false
	e := Bullet_End{id = index, owner = b.owner, shot = b.shot_id, weapon = b.weapon, pos = b.pos}
